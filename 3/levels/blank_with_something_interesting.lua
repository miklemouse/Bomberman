...
function levels.replace_me.load()
	require 'libs/Level'
	require 'libs/Map'
	require 'libs/Movable'
	require 'libs/Image'
	require 'libs/Behaviour'
	level = levelLib.new(50,{},mapLib.new ({'replace_me',ts=50}, imageLib.standartMapImages, mapLib.standartProbability, 13,13, 0, 0)) --not trash yet
...
function levels.replace_me.update(dt)
	...
	for iterator,v in pairs(level.map.cells) do 
		for i=1,level.map.mapX do for i2=1,level.map.mapY do
			...
			elseif iterator=='spawn' then if v[i][i2]=='player' then table.insert(level.movable,movableLib.new('player',{'w','a','s','d','space'},imageLib.standartImages,50,0,0)) level.map.cells.spawn=nil break end 
...
function levels.replace_me.draw()
	levelLib.draw()
end