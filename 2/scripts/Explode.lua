function explode(xb,yb,range,ft,num)
	u=xb/ts
	l=yb/ts
		...
		if S>0 then
			...
			if var == 0 or var == 'Fr' or var == 'Fm' or var == 'Fd' or var == 'Fl' or var == 'Fu' then
				if var~=arg and var~=0 then map[S][S2]='Fm' else map[S][S2]=arg end 
					fires[S][S2][1]=0 fires[S][S2][2]=ft territory[S][S2]=num
			elseif boolean then	a=math.random(90)
	...
	-- local function obsBool (ux,ly)
		...
		-- return false
	-- end
			map[u][l]='Fm' fires[u][l][2]=ft territory[u][l]=num 
